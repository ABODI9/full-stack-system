 ...
 */
export async function aiAsk({ userId, text, mode = 'local' }) {
    const q = (text ?? '').trim();
    if (!q)
        return { reply: 'Please write your question.' };
    ...
    const context = `UserID=${userId} | Records=${summary._count} | ` +
...
async function replyLocal(q, recordsCount, recordsTotal, orders, context) {
...
/** رد عبر OpenAI */
async function replyWithOpenAI(question, context) {
    try {
...
export async function aiHistory(userId, limit = 50) {
